...
# Load source
from src.config import colors,discrete_cmap,min_spot_intensity,sequential_cmap,site_names,edit_names,edit_ids
from src.config import img_paths, preedited_clone_names, site_ids, edit_palette, preedited_clone_colors
from src.utils import save_plot
from src.tree_utils import alleles_to_characters
from src.img_utils import load_fov, get_rgb, unsharp_mask
from src.legends import edit_legend
...
def plot_allele_heatmap(alleles,cells,int_order,legend = True,figsize = (3,4)):
    # Subset int_order to only include integrations in alleles
    present_ints = alleles.intID.unique()
    int_order = [i for i in int_order if i in present_ints]
    # Get characters
    characters = alleles_to_characters(alleles,edit_ids)
    characters["clone"] = cells["clone"]
    characters = characters.sort_values("clone",ascending=False)
    characters = characters.astype(str)
    # Make tdata
    tree = nx.DiGraph()
    tree.add_node("root",time = 0)
    for leaf in characters.index:
        tree.add_edge("root",leaf)
        tree.add_node(leaf,time = 1)
    tdata = td.TreeData(obs = characters,obst = {"tree":tree})
    # Plot
    fig, ax = plt.subplots(figsize = figsize,dpi = 300,layout = "constrained")
    pycea.pl.branches(tdata,linewidth=0)
    pycea.pl.annotation(tdata,keys=["clone"],width=3,palette = preedited_clone_colors,label = "Clone")
    edit_palette = {"-1":"white","0":"lightgray"}
    edit_palette.update({str(i):discrete_cmap[8][i-1] for i in range(1,9)})
    edit_palette.update({"9":"darkgray"})
    for i, integration in enumerate(int_order):
        pycea.pl.annotation(tdata,keys=[f"{integration}-EMX1",f"{integration}-HEK3",f"{integration}-RNF2"],border_width=.2,
                            label = [integration.replace("intID","")],width=1,gap = 2 if i == 0 else .5,palette = edit_palette)
    plt.xlabel("Target site integrations")
    plt.ylim(-.01,2*np.pi+.01)
    plt.ylabel("Cells")
    if legend:
        fig.legend(handles=edit_legend,loc='upper right',bbox_to_anchor=(.995,1),ncol=1)
    plt.xticks(fontsize=9)
    return fig     

def sc_allele_heatmap(plot_name,legend = True, figsize = (4.3,3.5)):
    alleles = pd.read_csv(data_path / "preedited_4T1_alleles.tsv",sep="\t", keep_default_na=False,index_col=0)
    alleles.index = alleles["cellBC"]
    cells = pd.read_csv(data_path / "preedited_4T1_cells.tsv",sep="\t", keep_default_na=False)
    cells.index = cells["cellBC"]
    whitelist = pd.read_csv(results_path / "preedited_clone_whitelist.tsv",sep="\t", keep_default_na=False,index_col=0)
    alleles = pd.read_csv(data_path / "preedited_4T1_alleles.tsv",sep="\t", keep_default_na=False,index_col=0)
    alleles["intID"] = alleles["intID"].map(whitelist.drop_duplicates(["intID","mfID"]).set_index("intID")["mfID"])
    int_order = whitelist.query("n_cells > 0").drop_duplicates("mfID")["mfID"]
    fig = plot_allele_heatmap(alleles,cells,int_order,legend,figsize)
    save_plot(fig,plot_name,plots_path,svg = False)

def allele_heatmap(experiment,plot_name,legend = True, figsize = (4.3,3.5)):
    spots = pd.read_csv(results_path / f"{experiment}_filtered_spots.csv",keep_default_na=False)
    spots = spots.rename(columns = {"cell":"cellBC"})
    spots.index = spots["cellBC"].astype(str)
    cells =gpd.read_file(results_path / f"{experiment}_cells_with_clone.json").rename(columns = {"cell":"cellBC"})
    cells.index = cells["cellBC"].astype(str)
    whitelist = pd.read_csv(results_path / "preedited_clone_whitelist.tsv",sep="\t", keep_default_na=False,index_col=0)
    int_order = whitelist.query("n_cells > 0").drop_duplicates("mfID")["mfID"]
    if experiment == "invivo":
        cells = cells.query("clone.isin([1,2])").copy()
        spots = spots.loc[cells.index].copy()
        int_order = whitelist.query("clone.isin([1,2]) & n_cells > 0").drop_duplicates("mfID")["mfID"]
    fig = plot_allele_heatmap(spots,cells,int_order,legend,figsize)
    save_plot(fig,plot_name,plots_path,svg = False)

def edit_representation(plot_name,figsize = (2,2)):
    alleles = pd.read_csv(data_path / "preedited_4T1_alleles.tsv",sep="\t", keep_default_na=False,index_col=0)
    edit_representation = alleles.melt(id_vars = "cellBC",var_name = "site",value_name = "edit",value_vars=edit_ids.keys())
    edit_representation = edit_representation.groupby(["site","edit"]).size().reset_index(name = "n_cells")
    edit_representation["id"] = edit_representation.apply(lambda x: edit_ids[x["site"]][x["edit"]],axis=1).astype(str)
    edit_representation["site_id"] = edit_representation["site"].map(site_ids)
    edit_representation = edit_representation.sort_values("id")
    fig, ax = plt.subplots(figsize = figsize,dpi = 300,layout = "constrained")
    sns.barplot(data = edit_representation,x = "site_id",y = "n_cells",hue = "id",palette = edit_palette,saturation=1)
    plt.ylabel("LM representation")
...
def clone_edit_whitelist(plot_name,figsize = (2,1)):
    whitelist = pd.read_csv(results_path / "preedited_clone_whitelist.tsv",sep="\t", keep_default_na=False,index_col=0)
    fig, ax = plt.subplots(1,3,figsize = figsize,dpi = 300,layout = "constrained")
    for i, site in enumerate(site_names.keys()):
        ...
        df = df[["clone","intID","column","value"]].pivot_table(
            index = "clone",columns = "column",values= "value",aggfunc = "first")
        cmap = mcolors.ListedColormap([list(edit_palette.values())[i] for i in range(1,10)])
        sns.heatmap(df,cmap = cmap,ax = ax[i],cbar = False)
    ...
    fig.suptitle("LM representation",y = 1.07)
...
if __name__ == "__main__":
    #sc_allele_heatmap("sc_allele_heatmap_no_legend",False,(4.3,3.5))
    #edit_representation("edit_representation",(2,2))
    #clone_edit_whitelist("clone_edit_whitelist",(2,1))
    for experiment in ["invitro","invivo"]:
        allele_heatmap(experiment,f"{experiment}_allele_heatmap_no_legend",False,(4.3,3.5))
