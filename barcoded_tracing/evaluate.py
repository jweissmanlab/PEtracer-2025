...
from petracer.tree import get_edit_frac, reconstruct_tree
...
def eval_fmi(param):
    """Evaluate the barcode FMI for a given set of parameters."""
    np.random.seed(param["iteration"])
    tdata = td.read_h5ad(data_path / f"barcoded_tracing_clone_{param['clone']}.h5td")
    # Sample characters
...
def get_mean_barcode_time(tdata,barcode):
    """Get the mean time for a given barcode LCA."""
    times = []
...
if __name__ == "__main__":
    ...
    clone_fmi(threads = petracer.config.threads)
    print("Testing how the number of characters affects the FMI")
    fmi_vs_characters(threads = petracer.config.threads)
    print("Testing how the detection rate affects the FMI")
    fmi_vs_detection(threads = petracer.config.threads)
