...
def edit_frac_simulation(num_simulations=100):
    results = []
    for edit_rate in np.linspace(0, .3, 31):
        edits = np.zeros((num_simulations,31,120), dtype=bool)
        for i in range(num_simulations):
            edited = np.zeros(120, dtype=bool)
            for j in range(31):
                edits[i,j] = (np.random.rand(120) < edit_rate) & (~edited)
                edited = edited | edits[i,j]
        for j in range(1,31):
            for k in range(1,120):
                has_edit = np.any(edits[:,:j,:k],axis = (2))
                edit_frac = np.sum(has_edit) / (j * num_simulations)
                results.append({"generations":j,"characters":k,
                                "edit_frac":edit_frac,"edit_rate":edit_rate})
    results = pd.DataFrame(results)
    return results 

# Simulate minimum number of characters for fraction of branches with edit
def min_characters_simulation(min_fracs = [.7,.8,.9], num_simulations = 10):
    results = []
    for iteration in range(num_simulations):
        edit_fracs = edit_frac_simulation()
        for min_frac in [.7,.8,.9]:
            min_characters = edit_fracs.query("edit_frac > @min_frac").groupby(
